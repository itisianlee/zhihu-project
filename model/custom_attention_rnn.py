...
class HierarchicalAttention:
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length, num_sentences,
        ...
        self.sen_batch_length = tf.placeholder(shape=(1,), dtype=tf.int32, name="sentence_batch_length")
    ...
    def gru_forward_word_level(self, embedded_words):
        ...
        h_t = tf.ones(self.batch_length)
        h_t = tf.reshape(h_t, [-1, self.hidden_size])  #TODO self.hidden_size h_t =int(tf.get_shape(embedded_words_squeeze[0])[0]) # tf.ones([self.batch_size*self.num_sentences, self.hidden_size]) # [batch_size*num_sentences,embed_size]
        h_t_forward_list = []
    ...
    def gru_backward_word_level(self, embedded_words):
        ...
        # h_t = tf.ones((self.batch_length * self.num_sentences, self.hidden_size))
        h_t = tf.ones(self.batch_length)
        h_t = tf.reshape(h_t, [-1, self.hidden_size])
        h_t_backward_list = []
    ...
    def gru_forward_sentence_level(self, sentence_representation):
        ...
        h_t = tf.ones(self.sen_batch_length)
        h_t = tf.reshape(h_t, [-1, self.hidden_size*2]) 
        h_t_forward_list = []
