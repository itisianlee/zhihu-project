...
class Utils():
        ...
        for epoch in range(self.epoch_num):
            ...
            for batch_num in range(num_batches_per_epoch):
                ...
                for i in xrange(reader.shape[0]):
                    ...
                    temp = temp1 + temp2
                    a, d = self.char_padding_sequence(temp)
                    x_char.append(a)
                    x_char_length.append(d)
                    
                    temp3 = reader.iloc[i][1].split(',')
                    temp4 = reader.iloc[i][3].split(',')
                    temp5 = temp3 + temp4
                    b, c = self.word_padding_sequence(temp5)
                    x_word.append(b)
                    x_word_length.append(c)
                    
                data = list(zip(x_char, x_char_length, x_word, x_word_length, y))
                data = np.array(data)
                yield data
    ...
    def generate_e_batch(self):
        ...
        while 1:
            ...
            data = list(zip(x_char, x_char_length, x_word, x_word_length, y, real_labels))    
            data = np.array(data)
            yield data
    ...
    def generate_p_batch(self):
        ...
        for batch_num in range(predict_set_num_batches):
            ...
            data = list(zip(x_char, x_char_length, x_word, x_word_length))    
            data = np.array(data)
            yield data
    ...
    def char_padding_sequence(self, xlist):
        ...
        if length >= self.chars_num:
            return rtn_list[0:self.chars_num], self.chars_num
        else:
